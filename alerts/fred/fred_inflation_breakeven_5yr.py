# Inflation 5-year breakeven rate - daily
...
# Load save file
saveFile = 'fred_inflation_breakeven_5yr.pickle'
...
# Pull data from URL
url = "https://fred.stlouisfed.org/graph/fredgraph.csv?id=T5YIE\
...
if lastAvailableDate < lastDate:
    ...
    dates = verifyDates(csv,dates,'DATE','T5YIE') # Verify dates are  weekdays and not weekends
    values = {
        'twoYearsAgo': getValueFromCSV(csv,dates['twoYearsAgo'].strftime('%Y-%m-%d'),'DATE','T5YIE'),
        'oneYearAgo': getValueFromCSV(csv,dates['oneYearAgo'].strftime('%Y-%m-%d'),'DATE','T5YIE'),
        'sixMonthsAgo': getValueFromCSV(csv,dates['sixMonthsAgo'].strftime('%Y-%m-%d'),'DATE','T5YIE'),
        'threeMonthsAgo': getValueFromCSV(csv,dates['threeMonthsAgo'].strftime('%Y-%m-%d'),'DATE','T5YIE'),
        'oneMonthAgo': getValueFromCSV(csv,dates['oneMonthAgo'].strftime('%Y-%m-%d'),'DATE','T5YIE'),
        'twoWeeksAgo': getValueFromCSV(csv,dates['twoWeeksAgo'].strftime('%Y-%m-%d'),'DATE','T5YIE'),
        'oneWeekAgo': getValueFromCSV(csv,dates['oneWeekAgo'].strftime('%Y-%m-%d'),'DATE','T5YIE'),
        'yesterday': getValueFromCSV(csv,dates['yesterday'].strftime('%Y-%m-%d'),'DATE','T5YIE'),
        'today': getValueFromCSV(csv,dates['today'].strftime('%Y-%m-%d'),'DATE','T5YIE'),
    }

    # Send tweet
    tweet = f"US 5-Year Breakeven Inflation Rate - {lastDate.strftime('%Y-%m-%d')}: {values['today']}%\n\
