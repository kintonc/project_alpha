# Inflation 10 year breakeven rate - daily
...
# Load save file
saveFile = 'fred_inflation_breakeven_10yr.pickle'
...
if lastAvailableDate < lastDate:
    ...
    dates = verifyDates(csv,dates,'DATE','T10YIE') # Verify dates are weekdays and not weekends
    values = {
        'twoYearsAgo': getValueFromCSV(csv,dates['twoYearsAgo'].strftime('%Y-%m-%d'),'DATE','T10YIE'),
        'oneYearAgo': getValueFromCSV(csv,dates['oneYearAgo'].strftime('%Y-%m-%d'),'DATE','T10YIE'),
        'sixMonthsAgo': getValueFromCSV(csv,dates['sixMonthsAgo'].strftime('%Y-%m-%d'),'DATE','T10YIE'),
        'threeMonthsAgo': getValueFromCSV(csv,dates['threeMonthsAgo'].strftime('%Y-%m-%d'),'DATE','T10YIE'),
        'oneMonthAgo': getValueFromCSV(csv,dates['oneMonthAgo'].strftime('%Y-%m-%d'),'DATE','T10YIE'),
        'twoWeeksAgo': getValueFromCSV(csv,dates['twoWeeksAgo'].strftime('%Y-%m-%d'),'DATE','T10YIE'),
        'oneWeekAgo': getValueFromCSV(csv,dates['oneWeekAgo'].strftime('%Y-%m-%d'),'DATE','T10YIE'),
        'yesterday': getValueFromCSV(csv,dates['yesterday'].strftime('%Y-%m-%d'),'DATE','T10YIE'),
        'today': getValueFromCSV(csv,dates['today'].strftime('%Y-%m-%d'),'DATE','T10YIE'),
    }

    # Send tweet
    tweet = f"US 10-Year Breakeven Inflation Rate - {lastDate.strftime('%Y-%m-%d')}: {values['today']}%\n\
